...
class GeneMatrix:
    ...
    def clustering(self):
        ...
        """
        import pandas
        df = pandas.read_csv('/labs/khatrilab/scottmk/scGAN-master/CS230_MS_LB/Data/clust.csv')
        clusts_manual = df.loc[:,'x']
        ...
        if self.cluster_res is None:
            ...
            #clustered.obs['manual'] = clusts_manual.values.astype('U')
            #sc_raw.obs['cluster'] = clustered.obs['louvain']
            #self.sc_raw.obs['cluster'] = clustered.obs['manual']
        ...
        if self.balanced_split:
            ...
            for key in valid_cells_per_cluster:
                ...
                test_valid_indices = np.random.choice(
                    ...
                    test_cells_per_cluster[key], replace=False) #changedd from fals
